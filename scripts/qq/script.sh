...
install_icalingua() {
    ...
    echo "==> 安装 icalingua (Snap)..."
    # 先移除旧版本（如有）
    sudo snap remove icalingua 2>/dev/null || true
    sudo snap install icalingua --classic
    
    echo ""
    echo "✅ QQ 安装完成！"
    echo "启动方式: snap run icalingua"
    echo "或在下拉菜单中搜索 'icalingua' 或 'QQ'"
    echo ""
    echo "💡 常见问题:"
...
echo "==> 选择安装方式..."
echo "1) Snap 版 icalingua (推荐，维护中)"
...
if [ "$choice" = "2" ]; then
    ...
    echo "==> 安装 QQ..."
    sudo apt install -y com.qq.im.deepin 2>/dev/null && {
        echo ""
